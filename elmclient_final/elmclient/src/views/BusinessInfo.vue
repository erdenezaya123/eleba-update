<template>
  <div class="wrapper">
    ...
    <cart-popup ref="popupRef" :style="{ bottom: '14vw' }">
      <div class="popup-container">
        <template v-if="cartArr.length !== 0">
          <div v-for="cart in cartArr" :key="cart.foodId" style="width: 100%">
            <div class="food">
              <div>
                <h3 class="name">{{ cart.foodName }}</h3>
...
export default {
  ...
  methods: {
    ...
    openCart() {
      this.cartArr = this.foodArr.filter(food => food.quantity && food.quantity > 0);
      console.log(this.cartArr);
      if (!this.cartArr || this.cartArr.length === 0) return;
      this.$refs.popupRef.openModal();
    },
  },
  computed: {
    //食品总价格
 ...
 */
 .popup-container {
	padding-bottom: calc(16vw * (1 / 3));
 }

 .popup-container .food {
	padding: 10px 20px;
	width: 100%;
	display: flex;
	font-size: 14px;
 }
 .popup-container .food div:first-child {
	flex: 1;
 }
 .popup-container .food div:last-child {
	flex: .5;
 }
 .popup-container .food .name {
	font-weight: normal;
	font-size: inherit;
 }

 .popup-container .food .price {
	color: rgb(239, 74, 9);
 }
</style>
